...
import {
  TrackType,
  type AnimationTrack,
  type AnimationTrackSet,
} from '../assets/loaders/AnimationLoader';
...
export async function playAnimation(id: string): Promise<void> {
  ...
  for (const track of Object.values(tracks)) {
    if (track.trackType !== TrackType.FLOAT)
      throw new Error(`AnimationPlayer: track type ${track.trackType} not implemented for "${id}"`);
  }
