...
export function detectRuntimeContext(): RuntimeContext {
  ...
  const signals = {
    ...
    'display-mode: window-controls-overlay': matchMedia('(display-mode: window-controls-overlay)')
      .matches,
    'navigator.standalone (iOS)':
      (navigator as unknown as { standalone?: boolean }).standalone === true,
    "'windowControlsOverlay' in navigator": 'windowControlsOverlay' in navigator,
    'UA: Chromium (desktop)':
      /Chrome|Chromium|Edg|OPR/.test(ua) && !/Android|iPhone|iPad|CriOS/.test(ua),
    ...
    'UA: iPhone/iPad':
      /iPhone|iPad/.test(ua) || (/Macintosh/.test(ua) && navigator.maxTouchPoints > 1),
  };
  ...
  if (import.meta.env.DEV) {
    ...
    void import('../../_dev/logger').then(({ createGroupLogger }) => {
