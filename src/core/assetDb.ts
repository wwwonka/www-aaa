...
export function openAssetDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!store.indexNames.contains(NAMESPACE_INDEX)) store.createIndex(NAMESPACE_INDEX, 'namespace')
...
export function getAssetsByNamespace(db: IDBDatabase, namespace: AssetNamespace): Promise<CachedAsset[]> {
  return new Promise((resolve, reject) => {
    const request = db.transaction(STORE_NAME, 'readonly').objectStore(STORE_NAME).index(NAMESPACE_INDEX).getAll(namespace)
...
export function getAssetsByType(db: IDBDatabase, type: string): Promise<CachedAsset[]> {
  return new Promise((resolve, reject) => {
    const request = db.transaction(STORE_NAME, 'readonly').objectStore(STORE_NAME).index(TYPE_INDEX).getAll(type)
    request.onsuccess = () => resolve(request.result as CachedAsset[])
    request.onerror   = () => reject(request.error)
  })
}
