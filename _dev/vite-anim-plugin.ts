...
function encodeAnim(tracks: Record<string, FlatAnimationTrack>): Buffer {
  ...
  for (const [id, track] of entries) {
    ...
    let offset = 0
    trackHeader.writeUInt8(idBytes.length, offset); offset += 1
    idBytes.copy(trackHeader, offset); offset += idBytes.length
    trackHeader.writeUInt8(0, offset); offset += 1 // TrackType.FLOAT
